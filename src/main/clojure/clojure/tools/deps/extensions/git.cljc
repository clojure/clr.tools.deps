...
(defn- coord-err
  ^#?(:clj Throwable :cljr Exception) [msg lib coord]
    ...
    (let [sha-dir (gitlibs/procure url lib sha)]
      (if sha-dir
        (let [root-dir (if root
                         #?(:clj (let [root-file (jio/file root)]
							...
							:cljr (let [root-file (cio/file-info root)]
							        ;; there is no notion of absolute vs nonabsolute for FileInfo.
									;; how do we make sure the sha-dir is in there?
									(.FullName root-file)))
                         sha-dir)]
          (if manifest
