...
(ns clojure.tools.deps.edn
  ...
  (:import
	#?(:clj [java.io BufferedReader File InputStreamReader PushbackReader Reader]
	   :cljr [System.IO  Path FileInfo DirectoryInfo TextReader])	
    [clojure.lang #?(:clj EdnReader$ReaderException :cljr EdnReader+ReaderException)
	              #?(:cljr PushbackTextReader)]
    ))
        ...
        (let [val (edn/read {:default tagged-literal :eof EOF} rdr)]
          (if (identical? EOF val)
            ...
            (if (not (identical? EOF (edn/read {:eof EOF} rdr)))
              ...
              val)))
        (catch #?(:clj EdnReader$ReaderException :cljr EdnReader+ReaderException) e
          (throw (io-err (str (#?(:clj .getMessage :cljr .Message) e) " (%s)") opts)))
        (catch #?(:clj RuntimeException :cljr Exception) t
          (if (str/starts-with? (#?(:clj .getMessage :cljr .Message) t) "EOF while reading")
            (throw (io-err "Error reading edn, delimiter unmatched (%s)" opts))
            (throw (io-err (str "Error reading edn. " (#?(:clj .getMessage :cljr .Message) t) " (%s)") opts))))))))
...
(defn user-deps
  ...
  return the deps.edn data"
  []
  (or (-> (user-deps-path) dir/canonicalize read-deps)
      (-> (user-deps-path) dir/canonicalize read-deps)))
...
(defn- choose-deps
  ...
  (cond
    ...
    (string? requested) (-> requested #?(:clj jio/file :cljr identity) dir/canonicalize read-deps)
